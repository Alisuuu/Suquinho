// --- API Functions, Core Logic, Filter Logic, Item Details Modal ---
        ...
        // --- FUNÇÕES DE FAVORITOS (CORRIGIDAS E ROBUSTAS) ---
        ...
        function saveFavorites(favs) {
            try {
                ...
                // Dispara um evento customizado para notificar a própria página da mudança
        ...
        function isFavorite(id, mediaType) {
            ...
            return currentFavorites.some(fav => fav.id == id && fav.media_type === mediaType);
        }

        function toggleFavorite(item, mediaType) {
            let currentFavorites = getFavorites();
            const index = currentFavorites.findIndex(fav => fav.id == item.id && fav.media_type === mediaType);
        ...
        function displayResults(items, defaultMediaType = null, targetGridElement, replace = true) {
            ...
            const currentFavorites = getFavorites(); // Pega a lista mais recente para a renderização
            items.forEach(item => {
                ...
                const isFav = currentFavorites.some(fav => fav.id == item.id && fav.media_type === mediaType);
                favButton.classList.toggle('active', isFav);
                favButton.innerHTML = isFav ? '<i class="fas fa-heart"></i>' : '<i class="far fa-heart"></i>';
                ...
                didOpen: () => {
                    document.querySelectorAll('.remove-favorite-button').forEach(button => {
                        button.addEventListener('click', (event) => {
                            ...
                            const itemToRemove = getFavorites().find(fav => fav.id === itemId && fav.media_type === mediaType);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // A variável `favorites` global não é mais necessária para lógica, mas pode ser mantida se quiser.
            // A boa prática é removê-la e usar getFavorites() sempre.
            ...
            // Ouve por mudanças de outras abas/janelas.
            window.addEventListener('storage', (event) => {
                if (event.key === FAVORITES_STORAGE_KEY) {
                    console.log("LOG: Mudança nos favoritos detectada em outra aba. Atualizando botões.");
                    document.querySelectorAll('.favorite-button[data-id]').forEach(btn => {
                        updateFavoriteButtonsState(parseInt(btn.dataset.id), btn.dataset.type);
                    });
                }
            });

             // Ouve por mudanças na própria página.
            window.addEventListener('favorites-updated', () => {
                 console.log("LOG: Mudança nos favoritos detectada na mesma aba. Atualizando botões.");
                 document.querySelectorAll('.favorite-button[data-id]').forEach(btn => {
                    updateFavoriteButtonsState(parseInt(btn.dataset.id), btn.dataset.type);
                });
            });
