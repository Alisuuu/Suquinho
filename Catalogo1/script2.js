...
async function openItemModal(itemId, mediaType, backdropPath = null) {
    ...
    // PLAYER_BASE_URL_MOVIE e PLAYER_BASE_URL_SERIES de Script 1
    ...
    currentExternalCopyUrl = linkForContentPage; // currentExternalCopyUrl de Script 1
    if(externalCopyButtonContainer) externalCopyButtonContainer.style.display = 'flex'; // externalCopyButtonContainer de Script 1
    const currentExternalBtn = document.getElementById('externalCopyLinkButton'); // externalCopyLinkButton de Script 1
    if(currentExternalBtn) {
        if (externalCopyButtonHandler) currentExternalBtn.removeEventListener('click', externalCopyButtonHandler); // externalCopyButtonHandler de Script 1
        externalCopyButtonHandler = () => { if(currentExternalCopyUrl) copyToClipboard(currentExternalCopyUrl, false); }; // copyToClipboard de Script 3
